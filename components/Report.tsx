...
const Report: React.FC<ReportProps> = ({
    ...
    return text.split("\n").map((line, index) => {
      if (line.startsWith("**") && line.endsWith("**")) {
        return (
          <h3
            key={index}
            className="text-xl font-bold text-slate-700 mt-6 mb-2"
          >
            {line.replace(/\*\*/g, "")}
          </h3>
        );
      ...
      if (line.trim().match(/^\d+\./)) {
        return (
          <li key={index} className="ml-5 list-decimal text-slate-600">
            {line.replace(/^\d+\.\s*/, "")}
          </li>
        );
      }
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in">
      <div
        id="report-container"
        className="bg-white p-10 md:p-12 rounded-2xl shadow-lg border border-slate-200"
      >
        <header className="text-center border-b-2 border-emerald-500 pb-4 mb-8">
          ...
          <p className="text-slate-500 text-lg mt-2">
            Personalizado para <span className="font-bold">{userName}</span>
          </p>
      ...
      <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
        <button
          onClick={onDownload}
          className="bg-emerald-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-emerald-700 transition duration-300"
        >
